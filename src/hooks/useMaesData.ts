...
export const mapDbToMae = (item: Record<string, unknown>): MaeProcessoComAtividade => ({
  ...
  cep: (item.cep as string) || null,
  endereco: (item.endereco as string) || null,
  numero: (item.numero as string) || null,
  complemento: (item.complemento as string) || null,
  bairro: (item.bairro as string) || null,
