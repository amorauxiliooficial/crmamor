  ...
  return (
    <div className="mb-1">
      <div className="flex items-center gap-1.5 px-3 py-1.5">
        ...
        <span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground/50">{title}</span>
        <span className="text-[9px] font-mono text-muted-foreground/30 ml-auto">{conversas.length}</span>
      </div>
      {conversas.map((c) => (
        <ConversaItem
          key={c.id}
...
function ConversaItem({ conversa: c, isSelected, isHovered, onSelect, onHover, onAssume, onPendente }: {
  ...
  return (
    <div
      className={cn(
        "group relative flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-all duration-150",
        isSelected
          ? "bg-primary/8 ring-1 ring-primary/15 rounded-lg mx-1"
          : "hover:bg-muted/30 mx-1 rounded-lg"
      )}
    ...
    >
      ...
      <div className="relative shrink-0">
        <Avatar className="h-8 w-8">
          <AvatarFallback className={cn(
            ...
            {c.nome ? c.nome.charAt(0).toUpperCase() : <User className="h-3 w-3" />}
          </AvatarFallback>
        </Avatar>
        {c.status === "Aberto" && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1">
          ...
          <span className="text-[9px] text-muted-foreground/40 shrink-0 tabular-nums font-mono">
        ...
        <div className="flex items-center gap-1 mt-0.5">
          <span className={cn("h-1 w-1 rounded-full shrink-0", STATUS_DOT[c.status])} />
          {c.prioridade === "alta" && (
            <Badge variant="destructive" className="h-3 text-[7px] px-1 py-0 font-bold rounded-full">
              🔥
            </Badge>
          )}
          {!c.atendente && (
            <span className="text-[8px] text-primary/60 font-medium">Sem atendente</span>
          )}
          {c.slaMinutos != null && c.status !== "Fechado" && (
            <span className={cn(
              "text-[8px] font-mono font-medium tabular-nums",
              c.slaMinutos <= 5 ? "text-destructive/70" : c.slaMinutos <= 15 ? "text-amber-600/60 dark:text-amber-400/60" : "text-muted-foreground/30"
            )}>
              {c.slaMinutos}m
            </span>
          )}
          {c.etiquetas.filter(e => e !== "Urgente").slice(0, 1).map((e) => (
            <Badge
              key={e}
              variant="outline"
              className="h-3 text-[7px] px-1 py-0 font-medium border-border/20 text-muted-foreground/50 rounded-full"
            >
              {e}
            </Badge>
          ))}
      ...
      {/* Unread count */}
      {c.naoLidas > 0 && (
        <span className="bg-primary text-primary-foreground h-4 min-w-4 px-1 flex items-center justify-center rounded-full text-[8px] font-bold shrink-0">
          {c.naoLidas}
        </span>
      )}
            ...
            {onAssume && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" variant="secondary" className="h-5 w-5 rounded-md"
...
export function InboxSidebar({
  ...
  return (
    <div className="w-full md:w-[320px] shrink-0 border-r border-border/40 flex flex-col h-full bg-background">
      {/* Header */}
      <div className="px-3 pt-3 pb-1.5 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <h1 className="font-semibold text-xs tracking-tight">Atendimento</h1>
          </div>
          <div className="flex items-center gap-0.5">
            <kbd className="hidden md:inline-flex items-center gap-0.5 rounded border border-border/40 bg-muted/20 px-1 py-0.5 text-[9px] text-muted-foreground/50 font-mono">
              ⌘K
            </kbd>
            <ThemeToggle className="h-6 w-6 rounded-md" />
            <Button variant="ghost" size="icon" className="h-6 w-6 rounded-md text-muted-foreground" onClick={onOpenConfig}>
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-7 h-7 text-[11px] rounded-md bg-muted/20 border-border/20 focus-visible:border-primary/30 transition-all"
          />
        </div>

        {/* Queue mode toggle + Tabs */}
        <div className="flex items-center gap-1.5">
          <div className="flex gap-0.5 bg-muted/20 rounded-md p-0.5 flex-1">
            {TABS.slice(0, 4).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-1 text-[9px] font-medium py-0.5 rounded transition-all duration-150",
                    isActive
                ...
                >
                  ...
                  {tab.value === "nao_lidas" && naoLidasCount > 0 && (
                    <span className="ml-0.5 bg-primary text-primary-foreground text-[7px] rounded-full px-1 font-bold">
                      {naoLidasCount}
                    </span>
                  )}
                </button>
        ...
        <div className="flex gap-0.5 overflow-x-auto scrollbar-none">
          {CHIPS.map((chip) => (
            <button
              ...
              className={cn(
                "px-1.5 py-0.5 rounded-full text-[9px] font-medium whitespace-nowrap transition-all border",
      ...
      <ScrollArea className="flex-1">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 px-4">
            ...
            <p className="text-[10px] font-medium text-muted-foreground/60">Nenhuma conversa</p>
          </div>
        ) : queueMode === "smart" ? (
          <div className="py-1">
            <QueueSection
      ...
      <div className="px-3 py-1.5 border-t border-border/20 flex items-center justify-between">
        <span className="text-[9px] text-muted-foreground/40">{filtered.length} conversas</span>
        <div className="flex items-center gap-2">
          {smartQueue.urgentes.length > 0 && (
            <span className="text-[9px] text-destructive/60 font-medium">🔥 {smartQueue.urgentes.length}</span>
          )}
          {naoLidasCount > 0 && (
            <span className="text-[9px] text-muted-foreground/40 font-mono">{naoLidasCount} não lidas</span>
          )}
        </div>
      </div>
    </div>
